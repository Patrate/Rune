 ...
 *
 * Generated by ChessBoardModel and RuneItemOverrideList. Contains information
 * about 1) The base model to be drawn; and 2) The number of chess pieces to be
 * drawn.
 *
 */
public class RuneFinalisedModel implements IBakedModel {

	public RuneFinalisedModel(IBakedModel i_parentModel, int grade) {
		parentModel = i_parentModel;
		gradeLevel = grade;
	}

	/**
	 * We return a list of quads here which is used to draw the chessboard. We do
	 * this by getting the list of quads for the base model (the chessboard itself),
	 * then adding an extra quad for every piece on the chessboard. The number of
	 * pieces was provided to the constructor of the finalised model.
	 *
	 * @param state
	 * @param side  which side: north, east, south, west, up, down, or null. NULL is
	 *              a different kind to the others see here for more information:
	...
	public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, Random rand) {
		...
		combinedQuadsList.addAll(getRuneQuads(gradeLevel));
...
//    FaceBakery.makeBakedQuad() can also be useful for generating quads.  See mbe04: AltimeterBakedModel
	...
	@SuppressWarnings({ "deprecation" })
	private List<BakedQuad> getRuneQuads(int numberOfPieces) {
		ResourceLocation gradeTextureRL = new ResourceLocation(RuneMain.MOD_ID, "textures/item/blank_wood_rune.png");

//		ResourceLocation gradeTextureRL = new ResourceLocation("minecraft:block/diamond_block");
		AtlasTexture blocksStitchedTextures = ModelLoader.instance().getSpriteMap()
				.getAtlas(AtlasTexture.LOCATION_BLOCKS);
		TextureAtlasSprite gradeTexture = blocksStitchedTextures.getSprite(gradeTextureRL);

		// if you want to use your own texture, you can add it to the texture map using
		// code similar to this in your ClientProxy:
//    @SubscribeEvent
...
//    }
		...
		// "builtin/generated" item, which are generated from the 2D texture by adding a
...
//		int col = 0;
		...
		float columnCentrePosition = 0.5f;
		float rowCentrePosition = 0.5f;

//		final int ITEM_RENDER_LAYER_NONE_SPECIFIED = -1;
		final int ITEM_RENDER_LAYER0 = 0;
//		final int ITEM_RENDER_LAYER1 = 1;

		// make a baked quad for each side of the chessboard i.e. front and back (south
		// and north)
		final float DELTA_FOR_OVERLAP = 0.001F; // add a small overlap to stop the quad from lying exactly on top of
												// the
		// existing face, which leads to "z-fighting" where the two quads
		// fight each other to be on top. looks awful.

		BakedQuad runeFront = createBakedQuadForFace(columnCentrePosition, 0.5f, rowCentrePosition, 0.5f,
				-DISTANCE_BEHIND_SOUTH_FACE + DELTA_FOR_OVERLAP, ITEM_RENDER_LAYER0, gradeTexture, Direction.SOUTH);
		BakedQuad runeBack = createBakedQuadForFace(columnCentrePosition, 0.5f, rowCentrePosition, 0.5f,
				-DISTANCE_BEHIND_NORTH_FACE + DELTA_FOR_OVERLAP, ITEM_RENDER_LAYER0, gradeTexture, Direction.NORTH);
		returnList.add(runeFront);
		returnList.add(runeBack);

		return returnList;
	}

	/**
	 * // Creates a baked quad for the given face. // When you are directly looking
	 * at the face, the quad is centred at [centreLR, centreUD] // The left<->right
	 * "width" of the face is width, the bottom<-->top "height" is height. // The
	 * amount that the quad is displaced towards the viewer i.e. (perpendicular to
	 * the flat face you can see) is forwardDisplacement // - for example, for an
	 * EAST face, a value of 0.00 lies directly on the EAST face of the cube. a
	 * value of 0.01 lies // slightly to the east of the EAST face (at x=1.01). a
	 * value of -0.01 lies slightly to the west of the EAST face (at x=0.99). // The
	 * orientation of the faces is as per the diagram on this page //
	...
	private BakedQuad createBakedQuadForFace(float centreLR, float width, float centreUD, float height,
			float forwardDisplacement, int itemRenderLayer, TextureAtlasSprite texture, Direction face) {
		float x1, x2, x3, x4;
		float y1, y2, y3, y4;
		float z1, z2, z3, z4;
		int packednormal;
		...
		// the order of the vertices on the face is (from the point of view of someone
		...
		final int minV = 0;
		final int maxV = 16;
		int[] vertexData1 = vertexToInts(x1, y1, z1, Color.WHITE.getRGB(), texture, maxU, maxV, lightMapValue,
				packednormal);
		int[] vertexData2 = vertexToInts(x2, y2, z2, Color.WHITE.getRGB(), texture, maxU, minV, lightMapValue,
				packednormal);
		int[] vertexData3 = vertexToInts(x3, y3, z3, Color.WHITE.getRGB(), texture, minU, minV, lightMapValue,
				packednormal);
		int[] vertexData4 = vertexToInts(x4, y4, z4, Color.WHITE.getRGB(), texture, minU, maxV, lightMapValue,
				packednormal);
	 ...
	 * @param color         RGBA colour format - white for no effect, non-white to
	 *                      tint the face with the specified colour
	 * @param texture       the texture to use for the face
	 * @param u             u-coordinate of the texture (0 - 16) corresponding to
	 *                      [x,y,z]
	 * @param v             v-coordinate of the texture (0 - 16) corresponding to
	 *                      [x,y,z]
	 * @param lightmapvalue the blocklight+skylight packed light map value
	 *                      (generally: set this to maximum for items)
	...
	private int[] vertexToInts(float x, float y, float z, int color, TextureAtlasSprite texture, float u, float v,
		...
		return new int[] { Float.floatToRawIntBits(x), Float.floatToRawIntBits(y), Float.floatToRawIntBits(z), color,
				Float.floatToRawIntBits(texture.getU(u)), Float.floatToRawIntBits(texture.getV(v)), lightmapvalue,
				normal };
	}

	/**
	 * Calculate the normal vector based on four input coordinates Follows minecraft
	 * convention that the coordinates are given in anticlockwise direction from the
	 * point of view of someone looking at the front of the face assumes that the
