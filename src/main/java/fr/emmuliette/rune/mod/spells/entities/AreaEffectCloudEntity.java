...
public class AreaEffectCloudEntity extends Entity {
	private static final DataParameter<Float> DATA_RADIUS = EntityDataManager.defineId(AreaEffectCloudEntity.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Integer> DATA_COLOR = EntityDataManager.defineId(AreaEffectCloudEntity.class,
			DataSerializers.INT);
	private static final DataParameter<Boolean> DATA_WAITING = EntityDataManager.defineId(AreaEffectCloudEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<IParticleData> DATA_PARTICLE = EntityDataManager
			.defineId(AreaEffectCloudEntity.class, DataSerializers.PARTICLE);
	...
	public AreaEffectCloudEntity(EntityType<? extends AreaEffectCloudEntity> areaEffectCloudEntity, World world) {
	...
	public AreaEffectCloudEntity(World world, double x, double y, double z) {
		this((EntityType<? extends AreaEffectCloudEntity>)ModEntities.AREA_EFFECT_CLOUD.get(), world);
		this.setPos(x, y, z);
	}
	...
	/*
	 * public void setPotion(Potion p_184484_1_) { this.potion = p_184484_1_; if
	 * (!this.fixedColor) { this.updateColor(); } }
	 */
	...
	private void updateColor() {
		 ...
		 * this.getEntityData().set(DATA_COLOR, 0); } else {
		 * this.getEntityData().set(DATA_COLOR,
	...
	public void tick() {
		...
		if (this.level.isClientSide) {
			...
			if (flag) {
				if (this.random.nextBoolean()) {
					for (int i = 0; i < 2; ++i) {
						...
						if (iparticledata.getType() == ParticleTypes.ENTITY_EFFECT) {
							...
							int l = j >> 8 & 255;
							int i1 = j & 255;
							this.level.addAlwaysVisibleParticle(iparticledata, this.getX() + (double) f3, this.getY(),
									this.getZ() + (double) f4, (double) ((float) k / 255.0F),
									(double) ((float) l / 255.0F), (double) ((float) i1 / 255.0F));
						} else {
							this.level.addAlwaysVisibleParticle(iparticledata, this.getX() + (double) f3, this.getY(),
									this.getZ() + (double) f4, 0.0D, 0.0D, 0.0D);
						}
					}
				}
			} else {
				...
				for (int k1 = 0; (float) k1 < f5; ++k1) {
					...
					if (iparticledata.getType() == ParticleTypes.ENTITY_EFFECT) {
						...
						int j2 = l1 >> 8 & 255;
						int j1 = l1 & 255;
						this.level.addAlwaysVisibleParticle(iparticledata, this.getX() + (double) f8, this.getY(),
								this.getZ() + (double) f9, (double) ((float) i2 / 255.0F),
								(double) ((float) j2 / 255.0F), (double) ((float) j1 / 255.0F));
					} else {
						this.level.addAlwaysVisibleParticle(iparticledata, this.getX() + (double) f8, this.getY(),
								this.getZ() + (double) f9, (0.5D - this.random.nextDouble()) * 0.15D, (double) 0.01F,
			...
			if (this.tickCount % 5 == 0) {
				...
				for (EffectInstance effectinstance1 : this.getEffects()) {
					list.add(new EffectInstance(effectinstance1.getEffect(), effectinstance1.getDuration() / 4,
							effectinstance1.getAmplifier(), effectinstance1.isAmbient(), effectinstance1.isVisible()));
				}
								...
								if (d2 <= (double) (f * f)) {
									...
									for (EffectInstance effectinstance : list) {
										if (effectinstance.getEffect().isInstantenous()) {
											effectinstance.getEffect().applyInstantenousEffect(this, this.getOwner(),
													livingentity, effectinstance.getAmplifier(), 0.5D);
										} else {
	...
	protected void readAdditionalSaveData(CompoundNBT p_70037_1_) {
		...
		if (p_70037_1_.contains("Particle", 8)) {
			try {
				...
				LOGGER.warn("Couldn't load custom particle {}", p_70037_1_.getString("Particle"),
