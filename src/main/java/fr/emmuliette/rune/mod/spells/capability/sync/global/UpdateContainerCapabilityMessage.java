package fr.emmuliette.rune.mod.spells.capability.sync.global;
...
public abstract class UpdateContainerCapabilityMessage<HANDLER, DATA> {
	...
	public UpdateContainerCapabilityMessage(final Capability<HANDLER> capability, @Nullable final Direction facing,
	...
	protected UpdateContainerCapabilityMessage(final Capability<HANDLER> capability, @Nullable final Direction facing,
			final int windowID, final int slotNumber, @Nullable final DATA capabilityData) {
		this.capability = capability;
		this.facing = facing;
		this.windowID = windowID;
		this.slotNumber = slotNumber;
	 ...
	 * @param capabilityDataDecoder A function that decodes a data instance from the
	 *                              buffer
	 * @param messageFactory        A function to create the message instance
	 * @param <HANDLER>             The capability handler type
	 * @param <DATA>                The data type written to and read from the
	 *                              buffer
	 * @param <MESSAGE>             The message type
	 * @return The decoded message
	 */
	protected static <HANDLER, DATA, MESSAGE extends UpdateContainerCapabilityMessage<HANDLER, DATA>> MESSAGE decode(
	 ...
	 * @param buffer                The packet buffer
	 * @param capabilityDataEncoder A function that encodes a data instance to the
	 *                              buffer
	 * @param <HANDLER>             The capability handler type
	 * @param <DATA>                The data type written to and read from the
	 *                              buffer
	 * @param <MESSAGE>             The message type
	 */
	protected static <HANDLER, DATA, MESSAGE extends UpdateContainerCapabilityMessage<HANDLER, DATA>> void encode(
			final MESSAGE message, final PacketBuffer buffer,
	 ...
	 * @param capabilityDataApplier A function that applies the capability data from
	 *                              a data instance to a capability handler instance
	 * @param <HANDLER>             The capability handler type
	 * @param <DATA>                The data type written to and read from the
	 *                              buffer
	 * @param <MESSAGE>             The message type
	 */
	protected static <HANDLER, DATA, MESSAGE extends UpdateContainerCapabilityMessage<HANDLER, DATA>> void handle(
			final MESSAGE message, final Supplier<NetworkEvent.Context> ctx,
		...
		ctx.get().enqueueWork(() -> {
			...
			/*
			 * else if (message.windowID == player.openContainer.windowId) { container =
			 * player.openContainer; }
			 */ else {
				return;
			}

			CapabilityContainerUpdateMessageUtils.applyCapabilityDataToContainerSlot(container, message.slotNumber,
					message.capability, message.facing, message.capabilityData, capabilityDataApplier);
		});
	...
	public interface MessageFactory<HANDLER, DATA, MESSAGE extends UpdateContainerCapabilityMessage<HANDLER, DATA>> {
		MESSAGE createMessage(@Nullable Direction facing, int windowID, int slotNumber, @Nullable DATA capabilityData);
	}
}