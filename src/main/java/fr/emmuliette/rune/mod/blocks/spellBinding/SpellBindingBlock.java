...
public class SpellBindingBlock extends Block {
	private static final ITextComponent CONTAINER_TITLE = new TranslationTextComponent("container.spellbinding");

	public SpellBindingBlock(AbstractBlock.Properties prop) {
		super(prop);
	}
	...
	public INamedContainerProvider getMenuProvider(BlockState blockState, World world, BlockPos blockPos) {
		return new SimpleNamedContainerProvider((containerId, playerInventory, postCall) -> {
			return new SpellBindingContainer(containerId, playerInventory, IWorldPosCallable.create(world, blockPos));
		}, CONTAINER_TITLE);
	}
}