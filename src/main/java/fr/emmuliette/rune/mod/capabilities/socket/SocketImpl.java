...
public class SocketImpl implements ISocket {
	...
	public void sync(ItemStack container) {
		container.getCapability(SocketCapability.SOCKET_CAPABILITY).ifPresent(c -> this.sync());
	}
	...
	@Override
	public void sync() {
		SyncHandler.sendToAllPlayers(new SocketPacket(this.toNBT()), ServerLifecycleHooks.getCurrentServer());
	}
}